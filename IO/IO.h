...
typedef char LongString[FGETS_MAXSIZE];
...
#define FSTAT_IS_CHARACTER_SPECIAL  3
#define FSTAT_IS_BLOCK_SPECIAL      4
...
#define IS_REGULAR_FILE(filename) (Fstat(filename)==FSTAT_IS_REGULAR_FILE)
#define IS_DIRECTORY(filename) (Fstat(filename)==FSTAT_IS_DIRECTORY)
#define IS_CHARACTER_SPECIAL(filename) \
    (Fstat(filename)==FSTAT_CHARACTER_SPECIAL)
#define IS_BLOCK_SPECIAL(filename) (Fstat(filename)==FSTAT_BLOCK_SPECIAL)
#define IS_FIFO(filename) (Fstat(filename)==FSTAT_IS_FIFO)
#define IS_SYMBOLIC_LINK(filename) (Fstat(filename)==FSTAT_IS_SYMBOLIC_LINK)
#define IS_SOCKET(filename) (Fstat(filename)==FSTAT_IS_SOCKET)
...
#define aopen(a)  fopen(CHARP(a), "a")
...
/* return the number of nonblank blocks in a string */
int nonblankcount(char *s);
...
typedef struct
{
    ...
    int *size;         /* number of indices in each category */
    int *cumulant;     /* cumulant of size[] */
    /* category i is from index[cumulant[i],cumulant[i+1]-1] */
    int *index;        /* the indices */
} MultiIndex;
