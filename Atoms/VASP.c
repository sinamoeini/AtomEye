...
#define data(i,j,k) (*(data+((i)%nc[0])+(((j)%nc[1])+((k)%nc[2])*nc[1])*nc[0]))
int main (int argc, char *argv[])
{
    Aapp_Define_Config;
    int i, j, k, nc[3];
    char linebuf[MAXLINESIZE], *CHGCAR_fname="CHGCAR", *POTCAR_fname="POTCAR";
    FILE *in, *out;
    double *s_shift, x_shift[3]={0}, thickness[3], *data=NULL;
    ...
    {
        printf ("\nPurpose: convert VASP CHGCAR or LOCPOT to "
                "XCrySDen XSF format:\n");
    ...
    if IS_DIRECTORY(CHGCAR_fname)
        CHGCAR_fname = str2(CHGCAR_fname, "/CHGCAR");
    Config_load_from_VASP
    ...
    fprintf(out, "Electron Density x Supercell Volume (%.5g A^3)\n",
            M3VOLUME(H));
    fprintf(out, "DATAGRID_3D_#1\n");
    
    for (i=0; i<7+np; i++) fscanf(in, " %[^\n]", linebuf);
    fscanf(in, "%d %d %d", nc, nc+1, nc+2);

    REALLOC(chg2xsf, data, nc[0]*nc[1]*nc[2], double);
    for (k=0; k<nc[2]; k++)
        for (j=0; j<nc[1]; j++)
            for (i=0; i<nc[0]; i++)
                fscanf(in, "%lf", &data(i,j,k));
...
#ifdef _chg_planar_avg
#define MAXLINESIZE 256
int main (int argc, char *argv[])
{
    Aapp_Define_Config;
    int i, j, k, nc[3], direction;
    char linebuf[MAXLINESIZE], *CHGCAR_fname="CHGCAR", *POTCAR_fname="POTCAR";
    FILE *in, *out=stdout;
    double x_shift[3]={0}, thickness[3], data, *avg=NULL, sum=0;
    ...
    {
        printf ("\nPurpose: do planar average of VASP CHGCAR or LOCPOT file\n"
                "         along direction 0, 1 or 2.\n\n");
        printf ("Usage: %s direction\n\t(CHGCAR, POTCAR in ./)\n\n", argv[0]);
        printf ("   or, %s CHGCAR_fname direction\n\t(POTCAR in ./)\n\n",
                argv[0]);
        printf ("   or, %s POTCAR_fname CHGCAR_fname direction\n\n", argv[0]);
        return (1);
    }
    if IS_DIRECTORY(CHGCAR_fname)
        CHGCAR_fname = str2(CHGCAR_fname, "/CHGCAR");
    
    direction = atoi(argv[argc-1]);
    if ( OUW(direction,3) ) pe("direction can only be 0, 1 or 2.\n");
    ...
    M3rowthicknesses (H, thickness);
    fprintf (out, "Volume = %.15g [A]\n", M3VOLUME(H));
    fprintf (out, "Thickness[%d] = %.15g [A]\n",
    ...
    REALLOC_ZERO(chg_planar_avg, avg, nc[direction], double);
    for (k=0; k<nc[2]; k++)
        for (j=0; j<nc[1]; j++)
            for (i=0; i<nc[0]; i++)
            {
                fscanf(in, "%lf", &data);
                if (direction == 0) avg[i] += data;
                else if (direction == 1) avg[j] += data;
                else avg[k] += data;
                sum += data;
            }
    fprintf(out, "Supercell average = %g\n\n", sum/(nc[0]*nc[1]*nc[2]));
    
    fprintf (out, "a = [\n");
    for (k=0; k<nc[direction]; k++)
    {
        fprintf(out, "%g %g\n", DOUBLE(k)/nc[direction],
                avg[k]*nc[direction]/(nc[0]*nc[1]*nc[2]));
    }
    Free(avg);
    fprintf (out, "];\n\n");
