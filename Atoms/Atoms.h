...
extern const double Supercell_s_Center[3];
...
#define XTAL_bCc   (XTAL_bCC+1)   /* 4-atom Bain path */
...
#define XTAL_aucu  (XTAL_Ni3Al+1) /* 2-atom */
#define XTAL_tial  XTAL_aucu
#define XTAL_AuCu  (XTAL_aucu+1)  /* cubic 4-atom */
#define XTAL_TiAl XTAL_AuCu
#define XTAL_ABSTRACTS_MAX  (XTAL_AuCu+1)
...
typedef struct
{
    ...
    VoronoiSite *v;
    int images[DIMENSION][2];  /* number of padding images taken */
    double *columnar_axis;  /* columnar axis vector */
    int columnar_idx;  /* columnar grain arrangement in direction columnar */
...
#define VoronoiSpec_BCC(nc,V) VoronoiSpec_rebuild(nc,Xtal_abstracts+XTAL_BCC,V)
#define VoronoiSpec_FCC(nc,V) VoronoiSpec_rebuild(nc,Xtal_abstracts+XTAL_FCC,V)
#define VoronoiSpec_Random(nc,V) VoronoiSpec_rebuild(nc,NULL,V)
#define VoronoiSpec_Free(V) { Free((V)->v); (V)->nv=0;  \
        (V)->columnar_axis=NULL;}
