...
(int cid, char *title, unsigned int width, unsigned int height)
{
    ...
    if (jw == AX_MAXWIN)    /* there is no officemate */
    {
        ...
        {
#if ( defined(_Darwin) || defined(_DarwinIntel386) )
            AX_videomode = AX_VIDEOMODE_NO_SHM;
#else            
                           ...
                           char *file_name)
{
    ...
    { /* allocate buffer */
        ...
        {
            buffer[3*i]   = ((AX_mem[iw].i2[i] & AX_rmask) >> rshift) * rmag + rmag-1;
            buffer[3*i+1] = ((AX_mem[iw].i2[i] & AX_gmask) >> gshift) * gmag + gmag-1;
            buffer[3*i+2] = ((AX_mem[iw].i2[i] & AX_bmask) >> bshift) * bmag + bmag-1;
        }
        for (i=0; i<AX_size[iw].height; i++)
            row_pointers[i] = (png_bytep)
    ...
    },
    {
        ...
        while (cinfo.next_scanline < cinfo.image_height)
        {
            for (i=0; i<cinfo.image_width; i++)
            {
                ...
                row_pointer[0][3*i]   = ((c & AX_rmask) >> rshift) * rmag + rmag-1;
                row_pointer[0][3*i+1] = ((c & AX_gmask) >> gshift) * gmag + gmag-1;
                row_pointer[0][3*i+2] = ((c & AX_bmask) >> bshift) * bmag + bmag-1;
            }
            jpeg_write_scanlines (&cinfo, row_pointer, 1);
        }
    },
