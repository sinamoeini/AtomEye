...
bool treatevent (int iw)
{
    ...
    switch (AX_event[iw].type)
    {
        ...
        case MapNotify:
            ...
            WindowUnMapped[iw] = 0;
#ifdef _CYGWIN
            /* CYGWIN X-server does not automatically recover image */
            return (TRUE);
#else
            return (FALSE);
#endif
        case KeyPress:
            ...
            switch ( (keysym=AXKeysym0(iw)) )
            {
                ...
                case XK_F12:
                    ...
                    else return (load_atom_color(iw));
                case XK_Tab:
                ...
                case XK_Page_Up:
                    ...
                    n[iw].atom_r_ratio *= (1+n[iw].delta);
                    change_atom_r_ratio(1+n[iw].delta);
                    return(TRUE);
                case XK_Page_Down:
                    if ( (n[iw].color_mode == COLOR_MODE_AUXILIARY) &&
                         (AXSHFT(iw) || AXCTRL(iw)) )
                ...
                case XK_t:
                         ...
                         (n[iw].color_mode == COLOR_MODE_AUXILIARY) )
                    {
                        ...
                        return (FALSE);
                              ...
                              (AXSHFT(iw)) )
                    {
                        return (typein_auxiliary_threshold(iw));
                    }
                ...
                case XK_Home:
                    ...
                    n[iw].bond_radius *= (1+n[iw].delta);
                    change_bond_radius(1+n[iw].delta);
                    return (TRUE);
                case XK_End:
                    if (AXSHFT(iw))
                    {
                    ...
                    n[iw].bond_radius /= (1+n[iw].delta);
                    change_bond_radius(1./(1+n[iw].delta));
                    return (TRUE);
                case XK_r:
                    if ( AXCTRL(iw) &&
                         (n[iw].color_mode == COLOR_MODE_AUXILIARY) )
                    {
                        ...
                        n[iw].auxiliary_thresholds_rigid = 0;
...
#if ( defined(_Darwin) ||                       \
      defined(_DarwinIntel386) )
		  if (AXSHFT(iw)) {if (AXCTRL(iw)) return(config_advance_first(iw));
                    return(config_advance(iw,-n[iw].glob_advance));}
#endif
