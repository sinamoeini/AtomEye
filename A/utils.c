...
{
    register int i,j;
    if (temporary_disable_bond || (n[iw].bond_mode==BOND_MODE_USER)) return;
    for (i=0; i<np; i++)
...
bool color_encode_auxiliary (int iw)
{
    ...
    if ( n[iw].auxiliary_idx != n[iw].old_auxiliary_idx - 100 )
    {
        n[iw].auxiliary_thresholds_rigid = 0;
        n[iw].old_auxiliary_idx = n[iw].auxiliary_idx + 100;
    }
    if (n[iw].bond_mode)
    {
...
bool typein_auxiliary_threshold (int iw)
{
    double threshold[2];
    char danswer[MAX_FILENAME_SIZE], *answer;
    if (n[iw].color_mode != COLOR_MODE_AUXILIARY)
    {
        printf ("Must be in AUXILIARY color mode to change "
    ...
    answer = readline_gets
        (str3("\n", 
         ...
         danswer);
    sscanf (answer, "%lf %lf", threshold+0, threshold+1);
    xterm_release_focus(iw);
    ...
    return (color_encode_auxiliary(iw));
} /* end typein_auxiliary_threshold() */
